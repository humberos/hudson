...
export LUNCH=omni_$SONYDEVICE-userdebug
...
repo init -u git://github.com/omnirom/android.git -b $REPO_BRANCH
...
time mka cookies # recoveryzip recoveryimage checkapi
...
if [[ "$INCREMENTAL" == "no" ]]
then
  cp $OUT/obj/PACKAGING/target_files_intermediates/omni_$SONYDEVICE-target_files-eng.$BUILD_USER_ID.zip ../PREVIOUS-omni_$SONYDEVICE-target_files-eng.$BUILD_USER_ID.zip
fi

if [[ "$INCREMENTAL" == "yes" ]]
then
  echo making incremental update
  chmod 775 $OUT/omni_$SONYDEVICE-ota-eng.jenkins.zip
  ./build/tools/releasetools/ota_from_target_files -i ../PREVIOUS-omni_$SONYDEVICE-target_files-eng.$BUILD_USER_ID.zip $OUT/obj/PACKAGING/target_files_intermediates/omni_$SONYDEVICE-target_files-eng.$BUILD_USER_ID.zip $OUT/incremental_omni_$SONYDEVICE-ota_update.zip
fi
# /archive

cp $OUT/omni*.zip* $WORKSPACE/archive/

if [[ "$INCREMENTAL" == "yes" ]]
then
  for f in $(ls $OUT/incremental*.zip*)
  do
